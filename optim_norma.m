function  [with_norma, without_norma] = optim_norma(param, nrows, ncols, ncolors, shape, originalrows, shiftedrows, simstep_max, dsRGB, dstwocol, dsnb, max_vertices, max_edges, totalvertices, totaledgelocations, rvcolors, rvedges, rplabelled_colors, rplabelled_edges, method)

% OPTIM_NORMA.m computes the objective function with given values of
% lambda and mu for the optimization of parameter values. Itself does not
% do optimization. The matrices for the simulation (i.e. vertices) are
% received from SIMULATION_M1.m or SIMULATION_M2.m and the moments needed
% for the optimization from SIMCALCS.m.
%
% Compared to OPTIM.m, the objective function is computed both with and
% without normalization with moments of the dataset. The latter is expected to
% converge to zero as grid size tends to infinity.
%
%
...
d_RGB_w=zeros(ncolors,1); d_RGB_wo=d_RGB_w;
d_nb_w=zeros(ncolors,1); d_nb_wo=d_nb_w;
%d_twocol=0;

% Squared distances between data set and simulations normalised (or not) by moments of experimental dataset.
for i=1:ncolors
    if dsRGB(i)~=0
        d_RGB_w(i)=((dsRGB(i)-avRGB(i))/dsRGB(i))^2;
        d_RGB_wo(i)=(dsRGB(i)-avRGB(i))^2;
    else
        d_RGB_wo(i)=(dsRGB(i)-avRGB(i))^2;
        d_RGB_w(i)=d_RGB_wo(i);
    end
    if dsnb(i)~=0
        d_nb_w(i)=((dsnb(i)-avnb(i))/dsnb(i))^2;
        d_nb_wo(i)=(dsnb(i)-avnb(i))^2;
    else
        d_nb_wo(i)=(dsnb(i)-avnb(i))^2;
        d_nb_w(i)=d_nb_wo(i);
    end
...
absd_w =[d_RGB_w; d_nb_w];
absd_wo=[d_RGB_wo; d_nb_wo];
% the output is not absd but these scalars
with_norma=sum(absd_w);
without_norma=sum(absd_wo);
end
