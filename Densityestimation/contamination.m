function [edgelist, components1]=contamination(edges, nrows, ncols, shiftedrows, originalrows)
...
%
% edges             simstep_max matrices of size (nrows, ncols, 3) matrix
%                   whose entries (i,j,k) are indicator variables of edges
%                   between well (i,j) and its neighbor to the (k=1) right,
%                   (k=2) right down, (k=3) left down.
% edgelist          List of the edges (connected neighbors) of size (4, no.
%                   of edges). Each column stores an edge (v_1, v_2) as
...
%                   sub2ind, ie. each vertex is indexed by a linear index.
...
%                   is that each well within a connected component will
...
% Analogue for right-down arrow
[i, j]=find(edges(shiftedrows(:),:,2));
edgelist=[edgelist [shiftedrows(i);j';shiftedrows(i)+1;j'+1]];

[i, j]=find(edges(originalrows(:),:,2));
edgelist=[edgelist [originalrows(i);j';originalrows(i)+1;j']];

% Analogue for left-down arrow
[i, j]=find(edges(shiftedrows(:),:,3));
edgelist=[edgelist [shiftedrows(i);j';shiftedrows(i)+1;j']];
[i, j]=find(edges(originalrows(:),:,3));
